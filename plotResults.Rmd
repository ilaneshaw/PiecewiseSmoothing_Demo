...
# #17x10yrAgeClasses
# outputFolderSpPreds <- checkPath(file.path(Paths$outputPath, "PS/17x10yrAgeClasses/spPreds/"))
# outputFolderSpPredsRasters <- checkPath(file.path(Paths$outputPath, "PS/17x10yrAgeClasses/spPredsRasters"), create= TRUE)
# saveLocation <- checkPath(file.path(Paths$outputPath, "PS/PSFigs/17x10yrAgeClasses"), create = TRUE)

# # #9x20yrAgeClasses
# outputFolderSpPreds <- checkPath(file.path(Paths$outputPath, "PS/9x20yrAgeClasses/spPreds/"))
# outputFolderSpPredsRasters <- checkPath(file.path(Paths$outputPath, "PS/9x20yrAgeClasses/spPredsRasters"), create = TRUE)
# saveLocation <- checkPath(file.path(Paths$outputPath, "PS/PSFigs/9x20yrAgeClasses"), create = TRUE)

# 5x40yrAgeClasses
outputFolderSpPreds <- checkPath(file.path(Paths$outputPath, "PS/5x40yrAgeClasses/spPreds/"))
outputFolderSpPredsRasters <- checkPath(file.path(Paths$outputPath, "PS/5x40yrAgeClasses/spPredsRasters"), create = TRUE)
...
locationSpRas <- checkPath(file.path(Paths$inputPath, "spRasterFiles"))
...
ggsave(file.path(saveLocation, "landscapeSummary.jpeg"), landscapeSummary, width = 16, height = 20, units = "cm")
```
...
# TSD_all <- matrix(c(is, becomes),
#                  ncol=2,
#                  byrow = FALSE)
# TSD_all[TSD_all[, 2] < 1, 2] <- NA
# print(TSD_all)

# #reclass disturbance raster 1
# TSDRas1 <- terra::classify(disturbanceRaster1, TSD_all)
...
spPreds1D <- lapply(spList, FUN = function(sp) {
  print(sp)
  spPreds1D <- as.data.frame(read_csv(paste(outputFolderSpPreds, "/", sp, "_spPreds1D_", .studyAreaName, "_.csv", sep = "")))
...
ggsave(file.path(saveLocation, "1DResults.jpeg"), plotMeanSpDensity, width = 20, height = 37, units = "cm")
...
spMatricies <- lapply(spList, FUN = function(sp) {
  # browser()
  spMatrix <- as.data.frame(read_csv(paste(Paths$outputPath, "/PS/17x10yrAgeClasses/spPreds/", sp, "_matrix_", .studyAreaName, "_.csv", sep = "")))
...
ggsave(file.path(saveLocation, "2DResults1.jpeg"), plotMeanSpDensity2D, width = 22, height = 30, units = "cm")
