...
sp.labs <- c(
  "Alder Flycatcher", "Bay-breasted Warbler", "Black-capped Chickadee",
  "Boreal Chickadee", "Brown Creeper", "Canada Jay", 
  "Cape May Warbler", "Common Yellowthroat", "Dark-eyed Junco", 
  "Golden-crowned Kinglet", "Least Flycatcher", "Mourning Warbler",
  "Ovenbird", "Palm Warbler", "Red-breasted Nuthatch",
  "Ruby-crowned Kinglet", "Red-eyed Vireo", "Swainson’s Thrush",
  "Tennessee Warbler", "Yellow-rumped Warbler"
)
...
spMatricies <- lapply(spList, FUN = function(sp) {
  ...
  spMatrix <- spMatrix %>% tidyterra::select(-1)
  print(spMatrix)
  return(spMatrix)
})
...
# get matricies
spMatricies <- lapply(spList, FUN = function(sp) {
  spMatrix <- as.data.frame(read_csv(paste(Paths$outputPath, "/PS/9x20yrAgeClasses/spPreds/", sp, "_matrix_", .studyAreaName, "_.csv", sep = "")))
  spMatrix <- spMatrix %>% tidyterra::select(-1)
  return(spMatrix)
})
names(spMatricies) <- spList
...
ggsave(file.path(Paths$outputPath, "PS/PSFigs/9x20yrAgeClasses/2DResults1.jpeg"), plotMeanSpDensity2D, width = 22, height = 30, units = "cm")
...
# get matricies
spMatricies <- lapply(spList, FUN = function(sp) {
  spMatrix <- as.data.frame(read_csv(paste(Paths$outputPath, "/PS/5x40yrAgeClasses/spPreds/", sp, "_matrix_", .studyAreaName, "_.csv", sep = "")))
  spMatrix <- spMatrix %>% tidyterra::select(-1)
  return(spMatrix)
})
names(spMatricies) <- spList
...
ggsave(file.path(Paths$outputPath, "PS/PSFigs/5x40yrAgeClasses/2DResults2.jpeg"), plotMeanSpDensity2D, width = 22, height = 30, units = "cm")
# plotMeanSpDensity2D#plotMeanSpDensityforClass2D
# jpeg("plotMeanSpDensity2D.jpg")
plotMeanSpDensity2D
```
...
gtTabs <- lapply(spList, FUN = function(sp) {
  ...
  spMatrix <- as.data.frame(read_csv(paste(outputFolderSpPreds, "/", sp, "_matrix_", .studyAreaName, "_.csv", sep = "")))
  spMatrix <- spMatrix %>% tidyterra::select(-1)
...
lapply(spList, FUN = function(sp) {
  ...
  spPreds1D <- as.data.frame(read_csv(paste(Paths$outputPath, "/PS/17x10yrAgeClasses/spPreds/", .studyAreaName, "_", sp, "_birdPreds1D", ".csv", sep = "")))
  #####################################################
  spPreds1D <- spPreds1D %>% tidyterra::select(-1)
  names(spPreds1D) <- c("FoLRaster", "landForClass", "classCount", "meanSpDensity", "medianSpDensity", "varSpDensity", "seSpDensity", "normalityStat", "normality_p", "unimodality_stat", "unimodality_p", "sp", "studyArea")
  print(spPreds1D)
  #########################################################

  spPreds1D <- spPreds1D[spPreds1D$FoLRaster == "forClass", ]
  spPreds1D <- spPreds1D[, c(2, 4)]
  ...
  spMatrix_10 <- as.data.frame(read_csv(paste(Paths$outputPath, "/PS/17x10yrAgeClasses/spPreds/", .studyAreaName, "_", sp, "_matrix", ".csv", sep = "")))
  ######################################
  # spMatrix_10 <-  spMatrix_10 %>% tidyterra::select(-1)
  ########################################
  print(spMatrix_10)
  ...
  spMatrix_20 <- as.data.frame(read_csv(paste(Paths$outputPath, "/PS/9x20yrAgeClasses/spPreds/", .studyAreaName, "_", sp, "_matrix", ".csv", sep = "")))
  ######################################
  # spMatrix_20 <-  spMatrix_20 %>% tidyterra::select(-1)
  ########################################
  ...
  spMatrix_40 <- as.data.frame(read_csv(paste(Paths$outputPath, "/PS/5x40yrAgeClasses/spPreds/", .studyAreaName, "_", sp, "_matrix", ".csv", sep = "")))
  ######################################
  #  spMatrix_40 <-  spMatrix_40 %>% tidyterra::select(-1)
  ########################################
  ...
  write.csv(spPreds1D, file = file.path(Paths$outputPath, "PS/PSFigs/", fileName))
  ...
  #   )
  # # browser()
  # gtsave(gtTab, paste(sp, "_AllPSResults.png", sep = ""), path = saveLocation, expand = 50)
})
...
lapply(spList, FUN = function(sp) {
  ...
  # spDatasets <- as.data.frame(read_csv(paste(outputFolderSpPreds, "/", sp, "_fullDataset_", .studyAreaName, ".csv", sep = "")))
  ...
  print(spHistogram)
  ggsave(file.path(saveLocation, paste(sp, "_histogram.jpeg", sep = "")), spHistogram, width = 20, height = 20, units = "cm")
})
```
...
lapply(spList, FUN = function(sp) {
  ...
  spDatasets <- as.data.frame(read_csv(paste(outputFolderSpPreds, "/", .studyAreaName, "_", sp, "_fullDataset", ".csv", sep = "")))
  #  spDatasets <- as.data.frame(read_csv(paste(outputFolderSpPreds, "/", sp, "_fullDataset_", .studyAreaName, "_.csv", sep = "")))
  spDatasets <- spDatasets %>% tidyterra::select(-1)
  ...
  ggsave(file.path(saveLocation, paste(sp, "_kernelDensity.jpeg", sep = "")), densityPlot, width = 20, height = 20, units = "cm")
})
```
...
statsGBM <- as.data.table(read_csv(paste(outputFolderSpPreds, "/statsGBM_studyArea_AB_BCR6.shp.csv", sep = "")))
statsGBM <- statsGBM %>% tidyterra::select(-1)
names(statsGBM) <- c("NA", "sp", "FriedmansHStat", "relInfAge")
...
plotRelInf <- ggplot(
  ...
  geom_point() +
  geom_segment(aes(yend = sp, xend = 0)) +
...
test <- statsGBM %>%
  rowwise() %>%
  arrange(relInfAge) %>%
  mutate(sp = factor(sp, sp))


plotGBM <- ggplot(test) +
  geom_segment(aes(y = FriedmansHStat, yend = relInfAge, x = sp, xend = sp), color = "grey") +
  geom_point(aes(x = sp, y = relInfAge, colour = "Proportional Influence of Age"), stroke = 1.5, shape = 1, size = 3) +
  geom_point(aes(x = sp, y = FriedmansHStat, color = "Friedman's H Statistic"), shape = 16, size = 4) +
  xlab("") +
...
lapply(spList, FUN = function(sp) {
  ...
  nameSp <- spNames[sp, ]
  # browser()
  spPreds1D <- as.data.frame(read_csv(paste(outputFolderSpPreds, "/", .studyAreaName, "_", sp, "_birdPreds1D.csv", sep = "")))
  ############## TODO: REMOVE ####################################

  names(spPreds1D) <- c("NA", "FoLRaster", "landForClass", "classCount", "meanSpDensity", "medianSpDensity", "varSpDensity", "seSpDensity", "normalityStat", "normality_p", "unimodality_stat", "unimodality_p", "sp", "studyArea")
  spPreds1D <- spPreds1D %>% tidyterra::select(-1)
  print(spPreds1D)

  ####################################################################
  spPreds1D <- spPreds1D[, c(2, 8:11)]
  print(spPreds1D)
  ...
  print(spPreds1D)
  # fileName <- paste(sp, "_1DPredsandStats", sep = "")
  # write.csv(spPreds1D, file = file.path(Paths$outputPath, "PS/PSFigs/", fileName))
...
assumptionsByClass1D <- as.data.table(read_csv(paste(Paths$outputPath, "/PS/17x10yrAgeClasses/spPreds/assumptionsByClass1D.csv", sep = "")))
...
assumptionsByClass2D_10 <- as.data.frame(read_csv(paste(Paths$outputPath, "/PS/17x10yrAgeClasses/spPreds/assumptionsByClass2D.csv", sep = "")))
assumptionsByClass2D_10[assumptionsByClass2D_10 == "2"] <- "2_10"

assumptionsByClass2D_20 <- as.data.frame(read_csv(paste(Paths$outputPath, "/PS/9x20yrAgeClasses/spPreds/assumptionsByClass2D.csv", sep = "")))
assumptionsByClass2D_20[assumptionsByClass2D_20 == "2"] <- "2_20"

assumptionsByClass2D_40 <- as.data.frame(read_csv(paste(Paths$outputPath, "/PS/5x40yrAgeClasses/spPreds/assumptionsByClass2D.csv", sep = "")))
...
######################################
names(assumptionsByClass) <- c("NA", "landAgeClass", "noSp", "propSpsNormal", "propSpsUnimodal", "smoothingType")
...
#    spearmanStats
spearmanStats_10 <- as.data.frame(read_csv(paste(Paths$outputPath, "/PS/17x10yrAgeClasses/spPreds/spearmanStats.csv", sep = "")))
names(spearmanStats_10) <- c("species", "spearman1D", "spearman2D_10")

spearmanStats_20 <- as.data.frame(read_csv(paste(Paths$outputPath, "/PS/9x20yrAgeClasses/spPreds/spearmanStats.csv", sep = "")))
names(spearmanStats_20) <- c("species", "spearman1D", "spearman2D_20")

spearmanStats_40 <- as.data.frame(read_csv(paste(Paths$outputPath, "/PS/5x40yrAgeClasses/spPreds/spearmanStats.csv", sep = "")))
...
# resStats
resStats_10 <- as.data.frame(read_csv(paste(Paths$outputPath, "/PS/17x10yrAgeClasses/spPreds/resStats.csv", sep = "")))
names(resStats_10) <- c("species", "medianRes1D", "medianRes2D_10", "moran1D", "moran2D_10")

resStats_20 <- as.data.frame(read_csv(paste(Paths$outputPath, "/PS/9x20yrAgeClasses/spPreds/resStats.csv", sep = "")))
names(resStats_20) <- c("species", "medianRes1D", "medianRes2D_20", "moran1D", "moran2D_20")

resStats_40 <- as.data.frame(read_csv(paste(Paths$outputPath, "/PS/5x40yrAgeClasses/spPreds/resStats.csv", sep = "")))
...
spearmanStats_40 <- as.data.frame(read_csv(paste(Paths$outputPath, "/PS/9x20yrAgeClasses/spPreds/spearmanStats.csv", sep = "")))
names(spearmanStats_40) <- c("species", "spearman1D", "spearman2D_40")
...
resStats_40 <- as.data.frame(read_csv(paste(Paths$outputPath, "/PS/9x20yrAgeClasses/spPreds/resStats.csv", sep = "")))
names(resStats_40) <- c("species", "medianRes1D", "medianRes2D_40", "moran1D", "moran2D_40")
...
lapply(spList, FUN = function(sp) {
  ...
  mapRas <- terra::rast(paste(outputFolderSpPredsRasters, "/", sp, "_for2DAndLc1DMap_", .studyAreaName, sep = ""))
  resRas <- terra::rast(paste(outputFolderSpPredsRasters, "/", sp, "_for2DAndLc1DRes_", .studyAreaName, sep = ""))
...
lapply(spList, FUN = function(sp) {
  ...
  nameSp <- spNames[sp, ]
  nmRas <- terra::rast(paste(Paths$inputPath, "/spRasterFiles/", sp, "-meanBoot_BCR-60_studyArea_AB_BCR6", sep = ""))
  mapRas_1D <- terra::rast(paste(Paths$outputPath, "/PS/17x10yrAgeClasses/outputSpPredsRasters/", .studyAreaName, "_", sp, "_for1DAndLc1DMap", sep = ""))
  resRas_1D <- terra::rast(paste(Paths$outputPath, "/PS/17x10yrAgeClasses/outputSpPredsRasters/", sp, "-for1DAndLc1DRes", sep = ""))

  mapRas_2D_10 <- terra::rast(paste(Paths$outputPath, "/PS/17x10yrAgeClasses/outputSpPredsRasters/", .studyAreaName, "_", sp, "-for2DAndLc1DMap", sep = ""))
  resRas_2D_10 <- terra::rast(paste(Paths$outputPath, "/PS/17x10yrAgeClasses/outputSpPredsRasters/", sp, "-for2DAndLc1DRes", sep = ""))

  mapRas_2D_20 <- terra::rast(paste(Paths$outputPath, "/PS/9x20yrAgeClasses/outputSpPredsRasters/", .studyAreaName, "_", sp, "-for2DAndLc1DMap", sep = ""))
  resRas_2D_20 <- terra::rast(paste(Paths$outputPath, "/PS/9x20yrAgeClasses/outputSpPredsRasters/", sp, "-for2DAndLc1DRes", sep = ""))

  mapRas_2D_40 <- terra::rast(paste(Paths$outputPath, "/PS/5x40yrAgeClasses/outputSpPredsRasters/", .studyAreaName, "_", sp, "-for2DAndLc1DMap", sep = ""))
  resRas_2D_40 <- terra::rast(paste(Paths$outputPath, "/PS/5x40yrAgeClasses/outputSpPredsRasters/", sp, "-for2DAndLc1DRes", sep = ""))
